const TuliaEditor = function (selector, options) {
    ...
    options.include.stylesheets['_tulia_lead_font'] = 'https://fonts.googleapis.com/css2?family=Poppins:wght@100;300;400;700&display=swap';
...
TuliaEditor.i18n['en'] = {
    ...
    'edit': 'Edit',
    'emptyContent': 'Empty document :(',
    'startEditingContent': 'Edit, to start journey!',
};
TuliaEditor.i18n['pl'] = {
    'save': 'Zapisz',
    'cancel': 'Anuluj',
    ...
    'edit': 'Edytuj',
    'emptyContent': 'Nic tu jeszcze nie ma :(',
    'startEditingContent': 'Edytuj by rozpocząć przygodę!',
};
...
(function () {
    class Translator {
        ...
        trans (name, data, lang) {
            ...
            langs = this.resolveLocalesCodes(langs);
...
(function () {
    const preview = '<div class="tued-container tued-preview tued-preview-opened">'
            + '<div class="tued-preview-toolbar"><div class="tued-preview-headline"><span class="tued-logo">Tulia Editor</span> - podgląd treści</div><button type="button" class="tued-btn" data-tued-action="edit">Edit</button></div>'
    ...
    class Layout {
        ...
        createLayout (config) {
            ...
            this.preview.find('button[data-tued-action="edit"]').text(this.translator.trans('edit'));
    ...
    class Canvas {
        ...
        init () {
            ...
            this.element.ready(function () {
                self.body.empty();
                self.body.append('<div class="tued-structure-container"></div><div class="tued-tools-container"><div class="tued-add-block"><span>Dodaj blok</span></div></div>');
        ...
        get structureContainer () {
            return this.element.contents().find('body .tued-structure-container');
        };

        get toolsContainer () {
            return this.element.contents().find('body .tued-tools-container');
        };
...
(function () {
    class InsertBlockWithSection {
         ...
         * @param {TuliaEditor.Document.Application.Service.StructureManipulator.BlockManipulator} blockManipulator
         */
        constructor (canvas, blocksPicker, sectionManipulator, rowManipulator, blockManipulator) {
            /** @type {TuliaEditor.Layout.Infrastructure.View.Canvas} */
            this.canvas = canvas;
        ...
        handleInsertBlockWithSection () {
            let self = this;

            this.canvas.toolsContainer.find('.tued-add-block').click(function () {
                self.blocksPicker.open(function (block) {
                    self.blocksPicker.close();
                    self.insertSectionWithBlock(block);
                });
            });
        }

        insertSectionWithBlock (block) {
            let section = this.sectionManipulator.create();
            let row     = this.rowManipulator.createInto(section);

            this.blockManipulator.insertInside(block, row.columns[0]);
        }
    }

    TuliaEditor.ns('TuliaEditor.Document.Application.EventListener.Structure.InsertBlockWithSection', InsertBlockWithSection);
}());
...
(function () {
    class SectionManipulator {
         ...
         * @return {TuliaEditor.Document.Domain.Grid.Entity.Section}
         */
        createAfter (targetSection) {
            return this.createSection('after', targetSection);
        }
        ...
        createBefore (targetSection) {
            return this.createSection('before', targetSection);
        }
        ...
        createSection (place, target) {
            ...
            if (place === 'before') {
                this.document.insertSectionBefore(section, target);
            } else if (place === 'after') {
                this.document.insertSectionAfter(section, target);
            } else {
                this.document.insertSection(section);
            }
...
(function () {
    class RowManipulator {
        ...
        /**
         * @param {TuliaEditor.Document.Domain.Grid.Entity.Row} row
         * @return {TuliaEditor.Document.Domain.Grid.Entity.Row}
         */
        createAfter (row) {
            return this.createRow('after', row);
        }
         ...
         */
        createBefore (row) {
            return this.createRow('before', row);
        }
         ...
         */
        createInto (section) {
            return this.createRow('inside', section);
        }
...
(function () {
    class BlockManipulator {
        ...
        createBlock () {
            ...
            this.blocksPicker.open(function (block) {
                self.insert(block);
                self.blocksPicker.close();
            });
        }
        ...
        prepareHead () {
            ...
            this.view.addTag('<style>\
                ...
                */\
                ...
                .tued-node-section-placeholder:empty:before {display:block;position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);color:#dcdcdc;text-transform:uppercase;font-size:12px;font-family:\'Poppins\',Arial,Tahoma,Verdana,sans-serif;}\
                ...
                \
                .tued-structure-container {margin:0 0 25px 0;}\
                .tued-tools-container {text-align:center;margin:0 25px 25px 25px;padding:20px;border:1px dashed #ddd;border-radius:5px;}\
                .tued-add-block {display:inline-block;position:relative;width:60px;height:50px;border-radius:6px;background-color:#dedede;transition:.12s all;}\
                .tued-add-block:hover {cursor:pointer;background-color:#d2d2d2;}\
                .tued-add-block:before,\
                .tued-add-block:after {position:absolute;content:"";display:block;left:50%;top:50%;transform:translate(-50%,-50%);background-color:#fff;width:4px;height:20px;}\
                .tued-add-block:after {transform:translate(-50%,-50%) rotate(90deg)}\
                .tued-add-block span {opacity:0;position:absolute;left:50%;top:0;display:block;transform:translate(-50%,-115%);background-color:#222;color:#fff;padding:3px 5px;border-radius:3px;white-space:nowrap;pointer-events:none;font-size:13px;transition:.12s all;}\
                .tued-add-block:hover span {opacity:1;}\
            </style>');
...
(function () {
    class Grid {
         ...
         */
        render (structure) {
            let body = this.canvas.structureContainer;
...
(function() {
    class PreviewUpdater {
         ...
         * @param {Object} options
         */
        constructor (exporter, view, translator, options) {
            /** @type {TuliaEditor.Document.Application.Service.Exporter} */
            this.exporter = exporter;
        ...
        updatePreview () {
            ...
            if (! exported.content) {
                exported.content = '<div style="padding:50px;text-align:center;color:#888;font-size:23px;text-transform:uppercase;font-weight:300;font-family:\'Poppins\',Arial,Tahoma,Verdana,sans-serif;">'
                    + this.translator.trans('emptyContent')
                    + '<span style="color:#000;margin-top:10px;font-size:16px;display:block;text-transform:none">'
                        + this.translator.trans('startEditingContent')
                    + '</span>'
                + '</div>';
            }

            this.view.updateContent(exported.content);
            this.view.updateStylesheets(this.options.include.stylesheets);
            this.view.updateJavascripts(this.options.include.scripts);
        }
    }
...
TuliaEditor.services = $.extend({}, TuliaEditor.services, {
    ...
    'TuliaEditor.Document.Application.EventListener.Structure.InsertBlockWithSection': {
        'class': TuliaEditor.Document.Application.EventListener.Structure.InsertBlockWithSection,
        'arguments': [
            '@TuliaEditor.Layout.Infrastructure.View.Canvas',
            '@TuliaEditor.Block.Domain.Block.BlockPicker',
            '@TuliaEditor.Document.Application.Service.StructureManipulator.SectionManipulator',
        ...
        'tags': [
            {'name': 'event_listener', 'listen': 'layout.canvas.ready', 'method': 'handleInsertBlockWithSection'},
        ],
    },
