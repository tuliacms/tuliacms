...
class FilemanagerExtension extends AbstractExtension
{
    ...
    {
        $finder = $this->finderFactory->getInstance(ScopeEnum::SINGLE);
        $finder->setCriteria([
            'id__in' => $ids,
            'type'   => TypeEnum::IMAGE,
            'order_by'  => 'id',
    ...
    {
        $svg = $this->finderFactory->getInstance(ScopeEnum::SINGLE)->find($id, TypeEnum::SVG);
    ...
    {
        return (bool) $this->finderFactory->getInstance(ScopeEnum::SINGLE)->find($id, $type);
    }
}
